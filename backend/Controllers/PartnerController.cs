...
namespace debraApp.Controllers
{
    ...
    public class PartnerController : Controller
    {
        ...
        [ProducesResponseType(403)] // Forbidden status for non-active accounts
        public IActionResult CheckPassword([FromBody] CheckPasswordRequest request)
        {
            ...
            if (partner.Status != "Active")
                return StatusCode(403, "Account is not activated");
        ...
        [HttpPut("toggleStatus/{partnerID}")]
        [ProducesResponseType(404)]
        [ProducesResponseType(204)]
        [ProducesResponseType(500)]
        public IActionResult TogglePartnerStatus(int partnerID)
        {
            if (!_partnerRepository.PartnerExists(partnerID))
            {
                return NotFound("Partner not found");
            ...
            if (!_partnerRepository.UpdatePartner(partner))
            {
                ModelState.AddModelError("", "Something went wrong toggling the partner status");
                return StatusCode(500, ModelState);
            }
