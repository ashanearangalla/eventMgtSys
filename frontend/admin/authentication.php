...
if ($userResult['http_code'] == 200) {
    ...
    exit;
} elseif ($userResult['http_code'] == 401 || $userResult['http_code'] == 403) {
    // User authentication failed or account is not activated, redirect to login with error status
...
if ($partnerResult['http_code'] == 200) {
    // Partner authenticated successfully, store user email in session
    $_SESSION['user']['email'] = $email;
    header('Location: ../partner/partner.php');
    exit;
} elseif ($partnerResult['http_code'] == 401 || $partnerResult['http_code'] == 403) {
    // Partner authentication failed or account is not activated, redirect to login with error status
    $_SESSION['message'] = $partnerResult['response']['message'] ?? 'Error occurred during authentication';
    header('Location: ../login.php');
    exit;
}
